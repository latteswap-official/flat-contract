...
contract LPChainlinkAggregator is IChainlinkAggregator, Initializable {
  ...
  // Calculates the lastest exchange rate representing token value in BNB
  function latestAnswer() external view override returns (int256) {
    ...
    uint256 _px0 = (uint256(token0Oracle.latestAnswer()) * (2**112)) / 1e18; // in 2**112
    uint256 _px1 = (uint256(token1Oracle.latestAnswer()) * (2**112)) / 1e18; // in 2**112
    ...
    return int256((_totalValue * 1e18) / (2**112));
  }
}
