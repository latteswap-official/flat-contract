...
// solhint-disable not-rely-on-time
contract MockFlatMarketForLatteSwapLiquidationStrategy is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  using LatteConversion for Conversion;
  using SafeERC20Upgradeable for IERC20Upgradeable;
  ...
  function kill(
    ...
    (uint256 _sumDebtAmount, uint256 _sumDebtShare, uint256 _sumCollateralShare) = _prepareLiquidates(
      _users,
      _maxDebtShares,
      _to
    );
    ...
    )
  {
    ...
    // 3. Loop-through all users to check if we are able to liquidate
    for (uint256 i = 0; i < _users.length; i++) {
      address _user = _users[i];
      uint256 _maxUserDebtShare = _maxDebtShares[i];
      // 3.1. If user position is safe, skip, otherwise can liquidate
      if (_checkSafe(_user, _collateralPrice)) continue;
      // 3.2. prepare liquidate
    ...
    )
  {
    ...
    // TODO: add close factor
    _lessDebtShare = MathUpgradeable.min(_maxDebtUserShare, userDebtShare[_user]);
    ...
    // 5. If user's collateral is 0, but debtShare is not 0;
    // Then it is bad debt. Hence move user's debt share to treasury.
    // Treausry will settle bad debt later by surplus or liquidation fee
    if (userCollateralShare[_user] == 0 && userDebtShare[_user] != 0) {
    ...
    // 6 transfer a collateral share of a user to _to
    clerk.transfer(collateral, _user, _to, _collateralShare);
  }
  ...
  function executeLiquidationStrategy(
    ...
    _flashLiquidateStrategy.execute(collateral, flat, _to, _shareToMin, _shareFrom);
  }
}
